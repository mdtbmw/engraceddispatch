...
class DeliveryRepository(private val deliveryDao: DeliveryDao) {
    ...
    suspend fun syncDeliveriesFromFirestore(userId: String) {
        try {
            ...
            // Offline — Room data already available via flow
        }
    }
    ...
    suspend fun insert(delivery: Delivery): Long {
        ...
        try {
            val userId = FirebaseService.currentUser?.uid ?: return localId
