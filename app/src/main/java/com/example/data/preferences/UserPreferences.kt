...
class UserPreferences(private val context: Context) {
    companion object {
        ...
        // Rider-specific
        private val IS_RIDER = booleanPreferencesKey("is_rider")
        private val RIDER_TOKEN = stringPreferencesKey("rider_token")
        private val RIDER_IS_ONLINE = booleanPreferencesKey("rider_is_online")
        private val RIDER_BIKE_NUMBER = stringPreferencesKey("rider_bike_number")
        private val RIDER_BIKE_MODEL = stringPreferencesKey("rider_bike_model")
        private val RIDER_TOTAL_DELIVERIES = intPreferencesKey("rider_total_deliveries")
        private val RIDER_ZONE = stringPreferencesKey("rider_zone")
    }
    ...
    // ===== RIDER PREFERENCES =====
    val isRider: Flow<Boolean> = context.dataStore.data.map { it[IS_RIDER] ?: false }
    val riderToken: Flow<String?> = context.dataStore.data.map { it[RIDER_TOKEN] }
    val riderIsOnline: Flow<Boolean> = context.dataStore.data.map { it[RIDER_IS_ONLINE] ?: false }
    val riderBikeNumber: Flow<String> = context.dataStore.data.map { it[RIDER_BIKE_NUMBER] ?: "" }
    val riderBikeModel: Flow<String> = context.dataStore.data.map { it[RIDER_BIKE_MODEL] ?: "" }
    val riderTotalDeliveries: Flow<Int> = context.dataStore.data.map { it[RIDER_TOTAL_DELIVERIES] ?: 0 }
    val riderZone: Flow<String> = context.dataStore.data.map { it[RIDER_ZONE] ?: "Lagos Mainland" }

    suspend fun setRider(value: Boolean) { context.dataStore.edit { it[IS_RIDER] = value } }
    suspend fun saveRiderToken(token: String) { context.dataStore.edit { it[RIDER_TOKEN] = token } }
    suspend fun setRiderOnline(value: Boolean) { context.dataStore.edit { it[RIDER_IS_ONLINE] = value } }
    suspend fun saveRiderProfile(bikeNumber: String, bikeModel: String, zone: String) {
        context.dataStore.edit {
            if (bikeNumber.isNotEmpty()) it[RIDER_BIKE_NUMBER] = bikeNumber
            if (bikeModel.isNotEmpty()) it[RIDER_BIKE_MODEL] = bikeModel
            if (zone.isNotEmpty()) it[RIDER_ZONE] = zone
        }
    }
    suspend fun saveRiderStats(totalDeliveries: Int) { context.dataStore.edit { it[RIDER_TOTAL_DELIVERIES] = totalDeliveries } }
