...
class DeliveryRepository(private val db: AppDatabase) {
    val parcels: Flow<List<Parcel>> = db.parcelDao().getAllParcels().distinctUntilChanged()
    val transactions: Flow<List<Transaction>> = db.transactionDao().getAllTransactions().distinctUntilChanged()
    val addresses: Flow<List<AddressItem>> = db.addressDao().getAllAddresses().distinctUntilChanged()
    val notifications: Flow<List<NotificationItem>> = db.notificationDao().getAllNotifications().distinctUntilChanged()
    val aiDispatchLogs: Flow<List<AIDispatchDecisionLog>> = db.aiDispatchDao().getAllLogs().distinctUntilChanged()
