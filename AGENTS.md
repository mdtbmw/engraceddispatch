...
These rules are mandatory for every future change. They apply to Android, web admin, public site, emails, notifications, widgets, support messages, empty states, loading states, error states, logs that can appear in UI, and any customer/rider/admin-visible copy.

## 1. User-Facing Language Rule

Never expose developer instructions, coding instructions, implementation status, internal terminology, audit terminology, AI-agent terminology, test terminology, stack traces, raw service names, or engineering jargon in production UI unless it is explicitly required as a legitimate product feature for an authorized admin/developer diagnostics surface.

Ordinary users should never see wording such as:
...
- "TODO"
- "FIXME"
- raw package names, SHA fingerprints, stack traces, tokens, service-account details, or API responses

Use normal product copy. If a verification code was sent, say "Verification code sent." Do not say "Real OTP sent. Not simulated."

## 2. Interpretation Rule

Developer instructions describe what the application should accomplish. They are not UI copy. Never copy a user's implementation request, coding prompt, audit wording, or agent instruction directly into product text.

## 3. Real Feature Rule

If a feature is required to be real, implement the behavior properly. Do not announce that it is real, production-ready, not mocked, backend-connected, or fully implemented. The user should experience the working feature through the flow itself.

## 4. No Self-Congratulatory UI

Never create UI that tells customers, riders, admins, or public-site visitors that the application is fully implemented, fully audited, generated by AI, production-ready, properly integrated, or successfully coded. Product quality should be felt through behavior.

## 5. No Debug UI in Production

Never expose debug information, API responses, tokens, OTP values, internal IDs, stack traces, development flags, mock status, developer notes, sample data controls, test accounts, seed utilities, or raw error payloads to normal users. Developer-only diagnostics must be gated away from production users and role-scoped.

## 6. Product-First Rule

When a requirement is ambiguous, ask what would make sense to a normal customer, rider, dispatcher, support agent, finance user, or owner. Do not ask how to literally represent the developer instruction in the UI.

## 7. Icon Semantics Rule

An icon must perform the action users reasonably associate with it. A heart saves or opens favorites. A cart opens or modifies cart. A search icon searches. A bell opens notifications. A phone calls. Do not connect icons to unrelated destinations because an existing route is convenient.

## 8. Empty State Rule

Never leave a disabled, unavailable, failed, or empty feature looking accidentally broken. Every empty state must explain what happened, why it is empty, and what the user can do next. Distinguish no data, loading, disabled, offline, permission denied, search with no results, and server error.

## 9. UX Consistency Rule

Do not create one-off interaction patterns when an existing application pattern already exists. Keep copy, icons, spacing, headers, bottom sheets, dialogs, forms, buttons, loading states, empty states, notifications, and motion consistent across customer, rider, admin, public site and email surfaces.

## 10. Animation Rule

Use subtle, responsive micro-interactions where they improve feedback. Animations should feel fluid and intentional, never decorative for their own sake. Motion must not imply real progress unless a real state change or fresh telemetry exists.
